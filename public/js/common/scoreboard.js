...
function LSMbuildNFLbox() {
	...
	lsmNFLmatchups.forEach((matchup, matchupIndex) => {
		...
		teams.forEach(team => {
			...
			// (1) TEAM IMG
			NFLBoxParts.push(
				`<td class="ls-nfl-team-img"><img src="https://knuckleheadsff.com/images/shared/icons/nfl/${teamId}.svg" title="${teamId}" style="vertical-align:middle"><span class="lsm-team-abbr" data-role="abbr" style="vertical-align:middle">${teamId}</span></td>`
			);
							...
							if (playerScore && playerScore.id) {
								if (playerScore.status === "starter") {
									...
									if (playerInfo && hasOwn(LSpos_team_imgs, playerInfo.position)) {
										...
										parts.push(`<img src="https://knuckleheadsff.com/images/shared/icons/nfl/${playerInfo.team}.svg" /></div>`);
									} else {
										parts.push(`<div class="player-image" id="image_${playerScore.id}"><div class="player-position ${String(playerInfo.position || '').toLowerCase()}">${playerInfo.position}</div>`);
										if (injuryStatus !== "none" && showInjury) {
											parts.push(`<span title="Injury Status" class="inj_status">${injuryStatus}</span>`);
										}
										parts.push(`<img src="https://knuckleheadsff.com/api/player-photo?id=${playerScore.id}" onerror="this.src='https://www.mflscripts.com/playerImages_96x96/free_agent.png'" loading="lazy" decoding="async" width="96" height="96" /><img class="ls-team-logo" alt="${playerInfo.team}" src="https://knuckleheadsff.com/images/shared/icons/nfl/${playerInfo.team}.svg" /></div>`);
									}
									if (playerInfo.team === 'FA' && scoreVal === 0) {
										parts.push(`<div class="player-details-box free-agent"><div class="player-name"><h3><a href="player?L=${league_id}&P=${playerScore.id}" class="position_${playerInfo.position}" target="new" data-player-id="${playerScore.id}"${titleAttr}${dataNewsAttr}>${formatPlayerNameLSModule(playerInfo.name)}</a></h3></div><div class="game-info">Free Agent</div></div>`);
									} else if (playerInfo.team === 'FA' && scoreVal !== 0) {
										parts.push(`<div class="player-details-box hasStats"><div class="player-name"><h3><a href="player?L=${league_id}&P=${playerScore.id}" class="position_${playerInfo.position}" target="new" data-player-id="${playerScore.id}"${titleAttr}${dataNewsAttr}>${formatPlayerNameLSModule(playerInfo.name)}</a></h3></div><div class="game-info"><div class="game-status ${playerInfo.team}">Free Agent</div></div><div class="player-stats stats_${playerScore.id}">${playerInfo.position === 'Def' ? get_TEAMstats_str(playerInfo.team, false) : (playerInfo.position === 'Off' ? get_TEAMstats_str(playerInfo.team, true) : doLSMBoxLiveStats(playerScore.id))}</div></div>`);
									} else if (matchupContent[playerInfo.team] === undefined) {
										parts.push(`<div class="player-details-box free-agent"><div class="player-name"><h3><a href="player?L=${league_id}&P=${playerScore.id}" class="position_${playerInfo.position}" target="new" data-player-id="${playerScore.id}"${titleAttr}${dataNewsAttr}>${formatPlayerNameLSModule(playerInfo.name)}</a></h3></div><div class="game-info">Bye Week</div></div>`);
									} else {
										if (playerScore.playerStatus === "playing") {
											parts.push(`<div class="player-details-box"><div class="player-name"><h3><a href="player?L=${league_id}&P=${playerScore.id}" class="position_${playerInfo.position}" target="new" data-player-id="${playerScore.id}"${titleAttr}${dataNewsAttr}>${formatPlayerNameLSModule(playerInfo.name)}</a></h3></div><div class="game-info"><div class="game-status ${playerInfo.team}">${matchupContent[playerInfo.team]}</div></div><div class="game-time-${playerInfo.team}">${matchupGame[playerInfo.team]}</div><div class="player-stats stats_${playerScore.id}">${playerInfo.position === 'Def' ? get_TEAMstats_str(playerInfo.team, false) : (playerInfo.position === 'Off' ? get_TEAMstats_str(playerInfo.team, true) : doLSMBoxLiveStats(playerScore.id))}</div></div>`);
										} else {
											parts.push(`<div class="player-details-box"><div class="player-name"><h3><a href="player?L=${league_id}&P=${playerScore.id}" class="position_${playerInfo.position}" target="new" data-player-id="${playerScore.id}"${titleAttr}${dataNewsAttr}>${formatPlayerNameLSModule(playerInfo.name)}</a></h3></div><div class="game-info"><div class="game-status ${playerInfo.team}">${matchupContent[playerInfo.team]}</div></div><div class="player-stats stats_${playerScore.id}">${playerInfo.position === 'Def' ? get_TEAMstats_str(playerInfo.team, false) : (playerInfo.position === 'Off' ? get_TEAMstats_str(playerInfo.team, true) : doLSMBoxLiveStats(playerScore.id))}</div></div>`);
										}
									}
									if (lsUseProjections) {
...
/* Team logo clipped to the headshot, matching the rosters report rather than
   floating mid-row next to the score. .player-image is already positioned, so
   this just overlays the corner. The rules above size every img in here to a
   3.5rem circle, hence the explicit overrides. */
#LSscoringBox .player-image img.ls-team-logo {
  position: absolute;
  bottom: -0.15rem;
  right: -0.15rem;
  height: 1.45rem;
  ...
  z-index: 2;
  pointer-events: none;
  filter: drop-shadow(0 0 0.125rem rgba(0, 0, 0, 0.85));
}
/* The opposite franchise mirrors its row (.player-image gets order: 3), so the
   badge moves to the outer corner on that side too. */
#LSscoringBox .franchise-box:nth-child(even) .player-row .player-image img.ls-team-logo,
#LSscoringBox.avg-matchup .franchise-box + .franchise-box .player-row .player-image img.ls-team-logo {
  right: auto;
  left: -0.15rem;
}
